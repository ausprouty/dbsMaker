...
export function fromObjId(
  v,
  keys = ["slug", "code", "id", "key", "name", "title"]
) {
  const raw = unref(v);
...
// Variant is required; lowercase + whitelist [a-z0-9-]
export function normVariant(v) {
  var s = normId(v);

  // Treat missing / null-ish as default
